...
// Parse "[label](https://url)" OR a bare URL into {label, url}
function parseOfficialLink(raw) {
  if (!raw) return { label: "", url: "" };
  const md = String(raw).match(/^\s*\[([^\]]+)\]\(([^)]+)\)\s*$/);
  if (md) {
    let url = md[2].trim();
    if (!/^https?:\/\//i.test(url)) url = "https://" + url;
    return { label: md[1].trim(), url };
  }
  let url = String(raw).trim();
  if (!/^https?:\/\//i.test(url)) url = "https://" + url;
  try {
    const u = new URL(url);
    return { label: u.hostname.replace(/^www\./, ""), url };
  } catch {
    return { label: raw, url };
  }
}

// Eligibility match score — counts how many positive signals match the scheme
function matchScore(scheme, form) {
  let score = 50;
  const note = (scheme.eligibilityNote || "").toLowerCase();
  if (form.bplCard && note.includes("bpl")) score += 15;
  if (form.disability && note.includes("disab")) score += 20;
  if (form.pregnant && note.includes("pregn")) score += 20;
  if (form.isWidow && note.includes("widow")) score += 20;
  if (form.farmer && note.includes("farm")) score += 15;
  if (form.aadhaar) score += 3;
  if (form.bankAccount) score += 3;
  if (scheme.level && form.state && scheme.level === form.state) score += 25;
...
// Each question only shows if its `when(form)` returns true. The wizard walks
// through them one at a time and skips irrelevant ones based on prior answers.
// ─────────────────────────────────────────────────────────────────────────────
const QUESTIONS = [
  { id:"name", label:"What is your full name?", type:"text", placeholder:"Your full name", required:true },
  { id:"age", label:"How old are you?", type:"number", placeholder:"e.g. 32", required:true, min:0, max:120 },
  { id:"gender", label:"What is your gender?", type:"select", required:true,
    options:["Male","Female","Transgender"] },
  { id:"state", label:"Which state / UT do you live in?", type:"select", required:true,
    options: () => statesList },
  { id:"residenceType", label:"Do you live in a rural or urban area?", type:"select", required:true,
    options:["Rural","Urban"] },
  { id:"category", label:"What is your social category?", type:"select", required:true,
    options:["General","OBC","SC","ST","EWS","Minority"] },
  { id:"maritalStatus", label:"What is your marital status?", type:"select",
    options:["Single","Married","Widowed","Divorced","Separated"] },
  // Branch: widow flag
  { id:"isWidow", label:"Are you a widow?", type:"bool",
    when:f => f.gender==="Female" && (f.maritalStatus==="Widowed" || parseInt(f.age)>=40) },
  // Employment branches based on age
  { id:"employmentStatus", label:"What best describes your current work?", type:"select", required:true,
    options:f => {
      const a = parseInt(f.age)||0;
      if (a<18) return ["Student","Child (not in school)","Apprentice"];
      if (a>=60) return ["Retired","Farmer","Daily Wage Worker","Self-employed","Salaried (Private)","Salaried (Government)","Homemaker","Unemployed"];
      return ["Student","Unemployed","Daily Wage Worker","Agricultural Worker","Farmer",
        "Self-employed","Salaried (Private)","Salaried (Government)","Construction Worker",
        "MSME/Small Business","Homemaker","Migrant Worker","Artisan/Craftsperson"];
    }
  ...
  // Student branch
  { id:"courseName", label:"What course or class are you studying?", type:"text",
    placeholder:"e.g. 10th Standard, B.Tech",
    when:f => f.employmentStatus==="Student" },
  { id:"education", label:"Your highest education level?", type:"select",
    options:["No formal education","Primary (1–5th)","Middle (6–8th)","Secondary (9–10th)",
             "Senior Secondary (11–12th)","Diploma / ITI","Undergraduate","Postgraduate","PhD"],
    when:f => parseInt(f.age)>=10 },
  // Occupation for workers
  { id:"occupation", label:"What kind of work do you do?", type:"text",
    placeholder:"e.g. Farmer, Mason, Tailor, Street vendor",
    when:f => f.employmentStatus && f.employmentStatus!=="Student" && f.employmentStatus!=="Child (not in school)" },
  // Farmer branch
  { id:"farmer", label:"Are you a farmer?", type:"bool",
    when:f => !["Student","Salaried (Government)"].includes(f.employmentStatus) && parseInt(f.age)>=18 },
  { id:"landOwned", label:"How much land do you own (in acres)?", type:"number", min:0, placeholder:"0 if none",
    when:f => f.farmer || ["Farmer","Agricultural Worker"].includes(f.employmentStatus) },
  { id:"cropType", label:"What is your main crop?", type:"text", placeholder:"e.g. Rice, Wheat",
    when:f => f.farmer || ["Farmer","Agricultural Worker"].includes(f.employmentStatus) },
  // Migrant worker
  { id:"migrantWorker", label:"Are you a migrant worker?", type:"bool",
    when:f => parseInt(f.age)>=18 && !["Salaried (Government)","Retired","Student"].includes(f.employmentStatus) },
  // Women branches
  { id:"pregnant", label:"Are you currently pregnant or nursing?", type:"bool",
    when:f => f.gender==="Female" && parseInt(f.age)>=18 && parseInt(f.age)<=49 },
  { id:"numChildren", label:"How many children do you have?", type:"number", min:0, placeholder:"0",
    when:f => parseInt(f.age)>=18 },
  { id:"selfHelp", label:"Are you a member of a Self-Help Group (SHG)?", type:"bool",
    when:f => f.gender==="Female" && parseInt(f.age)>=18 },
  // Disability
  { id:"disability", label:"Do you have a certified disability?", type:"bool" },
  // Housing
  { id:"ownHouse", label:"Do you own a pucca house?", type:"bool", when:f => parseInt(f.age)>=18 },
  { id:"hasElectricity", label:"Does your home have electricity?", type:"bool", default:true },
  { id:"hasToilet", label:"Does your home have a toilet?", type:"bool", default:true },
  { id:"hasLPG", label:"Do you have an LPG gas connection?", type:"bool" },
  { id:"hasPotableWater", label:"Do you have access to clean drinking water?", type:"bool" },
  { id:"hasTapWater", label:"Do you have a piped tap water connection?", type:"bool" },
  // Documents
  { id:"aadhaar", label:"Do you have an Aadhaar card?", type:"bool" },
  { id:"bankAccount", label:"Do you have a bank account?", type:"bool" },
  { id:"hasAadhaarLinkedBank", label:"Is your bank account linked to Aadhaar?", type:"bool",
    when:f => f.bankAccount && f.aadhaar },
  { id:"hasCasteCert", label:"Do you have a caste certificate?", type:"bool",
    when:f => ["SC","ST","OBC"].includes(f.category) },
  { id:"hasIncomeCert", label:"Do you have an income certificate?", type:"bool" },
  // Financial
  { id:"annualIncome", label:"What is your total annual family income (₹)?", type:"number",
    placeholder:"e.g. 120000", required:true, min:0 },
  { id:"bplCard", label:"Do you have a BPL / Antyodaya card?", type:"bool" },
  { id:"rationCard", label:"What type of ration card do you have?", type:"select",
    options:["None / Don't have","APL (Above Poverty Line)","BPL (Below Poverty Line)","AAY (Antyodaya)","PHH (Priority Household)"] },
  { id:"hasPDSRation", label:"Do you currently receive PDS ration?", type:"bool",
    when:f => f.rationCard && f.rationCard!=="None / Don't have" },
  { id:"hasChronicIllness", label:"Does anyone in your family have a chronic illness?", type:"bool" },
  // Tech access
  { id:"hasSmartphone", label:"Do you have a smartphone?", type:"bool" },
  { id:"hasInternet", label:"Do you have internet access?", type:"bool",
    when:f => f.hasSmartphone },
];

const STORAGE_KEY = "scheme_finder_profile_v1";
...
export default function SchemeFinder() {
  ...
  if (typeof window !== "undefined" && !hasRestored) {
    ...
    if (saved && !schemes) {
      // Don't auto-apply; show a banner offering restore
    }
    // mark flag in next render
  }

  const set = (k,v) => setForm(f=>({...f,[k]:v}));

  // Build dynamic visible question list each render based on current answers
  const visibleQuestions = QUESTIONS.filter(q => !q.when || q.when(form));
  const totalSteps = visibleQuestions.length;
  const safeIndex = Math.min(qIndex, totalSteps - 1);
  ...
  function isAnswered(q) {
    if (!q) return true;
    const v = form[q.id];
    if (q.type === "bool") return true; // bools are always defined
  ...
  if (schemes !== null) {
    return <ResultsPage schemes={schemes} form={form} onReset={handleReset}/>;
  }

  const savedExists = typeof window !== "undefined" && !!loadSavedProfile() && qIndex===0 && !form.name;
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#f0fdf4 0%,#dcfce7 50%,#bbf7d0 100%)",
      ...
      <div style={{textAlign:"center",marginBottom:"1.8rem"}}>
        ...
        <p style={{color:"#4b7c59",fontSize:14,margin:0}}>
          120+ real schemes · Smart dynamic questions · Direct apply links
        </p>
      </div>
      ...
      <div style={{background:"#fff",borderRadius:20,boxShadow:"0 8px 40px rgba(0,0,0,0.10)",
        ...
        {savedExists && (
          <div style={{background:"#fffbeb",border:"1px solid #fde68a",borderRadius:10,
            ...
            <span>💾 Found a saved profile from last time.</span>
            <button onClick={restoreSaved} style={{padding:"6px 12px",borderRadius:8,border:"none",
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#6b7280",
            ...
            <span>QUESTION {safeIndex+1} OF {totalSteps}</span>
            <span>{progress}%</span>
          </div>
          <div style={{height:8,background:"#e5e7eb",borderRadius:10,overflow:"hidden"}}>
            <div style={{width:`${progress}%`,height:"100%",background:"linear-gradient(90deg,#16a34a,#15803d)",
              transition:"width 0.3s ease"}}/>
          </div>
        </div>

        {currentQ && <QuestionView q={currentQ} form={form} set={set} onEnter={handleNext}/>}

        <div style={{display:"flex",gap:12,marginTop:20}}>
          {safeIndex>0 && (
            <button onClick={()=>setQIndex(safeIndex-1)} style={{
              flex:1,padding:"12px 0",borderRadius:10,border:"1.5px solid #d1d5db",
              background:"transparent",fontSize:15,fontWeight:600,color:"#374151",cursor:"pointer"}}>
              ← Back
            </button>
          ...
          <button
            ...
            style={{flex:2,padding:"12px 0",borderRadius:10,border:"none",
              background:!isAnswered(currentQ)?"#d1d5db":"#16a34a",
              color:"#fff",fontSize:15,fontWeight:700,
              cursor:!isAnswered(currentQ)?"not-allowed":"pointer",
              boxShadow:isAnswered(currentQ)?"0 4px 14px rgba(22,163,74,0.35)":"none",
              transition:"all 0.2s"}}>
            {safeIndex >= totalSteps-1 ? "🔍 Find My Schemes →" : "Next →"}
          </button>
        </div>
        ...
        <div style={{marginTop:14,textAlign:"center"}}>
          <button onClick={()=>{
            ...
            background:"transparent",border:"none",color:"#16a34a",fontSize:12,fontWeight:600,
      ...
      <p style={{fontSize:11,color:"#6b7280",marginTop:16,textAlign:"center"}}>
        Profile saved locally in your browser only · Never sent to any server
      </p>
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
// QUESTION RENDERER — renders any single question definition
// ─────────────────────────────────────────────────────────────────────────────
function QuestionView({ q, form, set, onEnter }) {
  const value = form[q.id];
  const opts = typeof q.options === "function" ? q.options(form) : q.options;

  return (
    <div>
      <div style={{fontSize:"clamp(1.05rem,2.4vw,1.3rem)",fontWeight:800,color:"#14532d",
        lineHeight:1.35,marginBottom:14}}>
        {q.label} {q.required && <span style={{color:"#dc2626"}}>*</span>}
      </div>

      {q.type === "text" && (
        <input style={inp} value={value || ""} placeholder={q.placeholder || ""}
          autoFocus
      ...
      {q.type === "number" && (
        <input style={inp} type="number" value={value || ""} placeholder={q.placeholder || ""}
          min={q.min} max={q.max} autoFocus
          onChange={e=>set(q.id, e.target.value)}
          onKeyDown={e=>{ if (e.key === "Enter") onEnter(); }}/>
      )}

      {q.type === "select" && (
        <div style={{display:"grid",gridTemplateColumns:opts.length>6?"1fr 1fr":"1fr",gap:8}}>
          {opts.map(o => {
            const selected = value === o;
            return (
              <button key={o} onClick={()=>set(q.id, o)} style={{
                ...
                background:selected?"#f0fdf4":"#fff",color:selected?"#14532d":"#374151",
      ...
      {q.type === "bool" && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            ...
            return (
              <button key={String(opt.v)} onClick={()=>set(q.id, opt.v)} style={{
                padding:"14px 0",borderRadius:10,fontSize:15,fontWeight:700,
                border:selected?"2px solid #16a34a":"1.5px solid #e5e7eb",
                background:selected?(opt.v?"#f0fdf4":"#fef2f2"):"#fff",
                color:selected?(opt.v?"#14532d":"#991b1b"):"#374151",
                cursor:"pointer",transition:"all 0.15s"}}>
                {opt.l}
              </button>
            );
...
// RESULTS PAGE
// ─────────────────────────────────────────────────────────────────────────────
function ResultsPage({schemes, form, onReset}) {
  const [selCat, setSelCat] = useState("All");
  const [expanded, setExpanded] = useState(null);
  const [search, setSearch] = useState("");
  const [sortBy, setSortBy] = useState("match"); // match | name | category

  const allSchemes = schemes.map(s => ({
    ...s,
    source: s.level === "Central" ? "central" : "state",
    _score: matchScore(s, form),
  }));
  ...
  if (sortBy === "match") filtered = [...filtered].sort((a,b)=>b._score - a._score);
  ...
  const totalBenefit = schemes.length;
  ...
  async function handleShare() {
    const text = `I'm eligible for ${totalBenefit} government schemes! Check yours at this Scheme Finder.`;
    if (typeof navigator !== "undefined" && navigator.share) {
      try { await navigator.share({ title: "My Eligible Schemes", text, url: window.location.href }); } catch {}
      ...
      <div style={{maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{textAlign:"center",marginBottom:"1.2rem"}}>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:12,flexWrap:"wrap"}}>
            <button onClick={handleShare} style={actionBtn("#16a34a")}>📤 Share</button>
            <button onClick={handlePrint} style={actionBtn("#1e3a8a")}>🖨️ Print</button>
            <button onClick={onReset} style={actionBtn("#92400e")}>🔄 Start Over</button>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:"1rem"}}>
          {[
            ["Central Schemes", centralCount, "#166534","#dcfce7"],
            ["State Schemes", stateCount, "#1e3a8a","#dbeafe"],
            ["Categories", new Set(allSchemes.map(s=>s.category)).size, "#4c1d95","#ede9fe"],
            ["Total Benefits", totalBenefit, "#92400e","#fef3c7"],
          ].map(([l,v,c,bg])=>(
            <div key={l} style={{background:bg,borderRadius:12,padding:"10px 12px",textAlign:"center",border:`1px solid ${bg}`}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:"0.8rem",flexWrap:"wrap"}}>
          ...
          {[["match","🎯 Best Match"],["name","🔤 Name"],["category","📂 Category"]].map(([k,l])=>(
            <button key={k} onClick={()=>setSortBy(k)} style={{
              padding:"4px 12px",borderRadius:16,fontSize:11,fontWeight:600,
              border:sortBy===k?"none":"1px solid #d1d5db",
              background:sortBy===k?"#15803d":"#fff",
              color:sortBy===k?"#fff":"#374151",cursor:"pointer"}}>
              {l}
            </button>
          ))}
        </div>
...
function SchemeCard({scheme,expanded,onToggle}) {
  const bg = categoryBg[scheme.category]||"#f3f4f6";
  const col = categoryColors[scheme.category]||"#374151";
  const { url: officialUrl, label: officialLabel } = parseOfficialLink(scheme.officialLink);
  const score = scheme._score;

  function copyLink(e) {
    e.stopPropagation();
    if (typeof navigator !== "undefined" && navigator.clipboard && officialUrl) {
      navigator.clipboard.writeText(officialUrl);
      alert(`Copied: ${officialUrl}`);
    }
  }
    ...
    <div style={{background:"#fff",borderRadius:14,border:`1px solid ${expanded?"#16a34a":"#e5e7eb"}`,
      ...
      <button onClick={onToggle} style={{width:"100%",display:"flex",alignItems:"center",gap:14,
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,flexShrink:0}}>
          <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"flex-end"}}>
            {typeof score === "number" && (
              <span style={{background:score>=80?"#16a34a":score>=60?"#f59e0b":"#6b7280",
                color:"#fff",fontSize:10,fontWeight:800,
                padding:"2px 8px",borderRadius:20,whiteSpace:"nowrap"}}>{score}% match</span>
            )}
            <span style={{background:bg,color:col,fontSize:10,fontWeight:700,
              padding:"2px 8px",borderRadius:20,whiteSpace:"nowrap"}}>{scheme.category}</span>
            {scheme.source==="state" && (
          ...
          {officialUrl && (
            <div style={{marginTop:14,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
              <a href={officialUrl} target="_blank" rel="noopener noreferrer" style={{
                display:"inline-flex",alignItems:"center",gap:6,color:"#fff",fontSize:13,
                fontWeight:700,textDecoration:"none",background:"#16a34a",
                borderRadius:8,padding:"10px 18px",
                boxShadow:"0 4px 14px rgba(22,163,74,0.35)"}}>
                🚀 Apply Now on Official Portal ↗
              </a>
              <a href={officialUrl} target="_blank" rel="noopener noreferrer" style={{
                display:"inline-flex",alignItems:"center",gap:6,color:"#16a34a",fontSize:12,
                fontWeight:600,textDecoration:"none",border:"1.5px solid #16a34a",
                borderRadius:8,padding:"8px 12px"}}>
                🌐 {officialLabel}
              </a>
              <button onClick={copyLink} style={{
                display:"inline-flex",alignItems:"center",gap:6,color:"#374151",fontSize:12,
                fontWeight:600,background:"#f3f4f6",border:"1px solid #e5e7eb",
